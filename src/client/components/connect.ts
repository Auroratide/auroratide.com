/**
 * Custom wrapper for creating custom elements out of svelte components.
 * 
 * This comes from https://github.com/crisward/svelte-tag. I've duplicated it in case
 * I need to update it, as the original repo may not be maintained.
 * 
 * Svelte custom elements have some crippling restrictions, namely:
 *   * Must always use the Shadow Dom, meaning you cannot cascade styles where desired
 *   * All inner components must also be defined as custom elements
 * 
 * Several issues have been made crying for some kind of optional flag, but it seems to
 * have fallen on deaf ears. Here are some issues:
 *   * https://github.com/sveltejs/svelte/issues/1748
 *   * https://github.com/sveltejs/svelte/issues/2605
 */

import type { SvelteCustomElement } from './SvelteCustomElement';
...
export default function(component: SvelteCustomElement, opts: ConnectionOptions = {
  ...
  class Wrapper extends HTMLElement {
    ...
    static get observedAttributes() {
      return component.customElement.props || []
    }
    ...
    connectedCallback(){
      ...
      this.elem = new component({	target: this._root as Element,	props});
    }
        ...
        if(mutation.type=="childList"){
          ...
          if(Object.keys(slots).length){
            ...
            if(this.slotcount != Object.keys(slots).length){
              ...
              this.slotcount = Object.keys(slots).length
              root.innerHTML = ""
              this.elem = new component({	target: root,	props});
            }
          }
        }
      }
  ...
  window.customElements.define(component.customElement.name, Wrapper);
}
