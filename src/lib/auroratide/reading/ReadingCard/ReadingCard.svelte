<script lang="ts">
	...
	import type { SummarizedReading } from "../ReadingType"

	export let value: SummarizedReading
</script>
...
<article aria-labelledby="{value.id}" class="{Theme(value.color)} lift-on-focus align-to-grid">
	<section class="{Color.bg.bg.b} {Color.border.primary.a} border-after column">
		<h2 id="{value.id}" class="balance no-margin">
			<a href="{Routes.Reading.href(value.id)}" class="{Color.text.fg.b} no-underline card-link">{value.title}</a>
		</h2>
		<p class="no-margin space-above small relative row align-start">By {value.author}{#if value.publisher}{` on ${value.publisher}`}{/if}</p>
		<div class="icon-bg {Color.text.primary.a}" aria-hidden="true">
			<vector-icon icon="{value.icon}"></vector-icon>
		</div>
	</section>
	<section class="summary {Color.bg.bg.b}">
		<p class="no-margin">{value.summary.long}</p>
	</section>
</article>
