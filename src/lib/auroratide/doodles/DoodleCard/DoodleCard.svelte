<script lang="ts">
	...
	import type { DoodleMetadata } from "../DoodleMetadata"

	export let value: DoodleMetadata
</script>
...
<article aria-labelledby="{value.id}" class="{Theme(value.color)} lift-on-focus align-to-grid">
	<section class="{Color.bg.bg.b} {Color.border.primary.a} border-after column">
		<h2 id="{value.id}" class="balance no-margin">
			<a href="{Routes.Doodles.href(value.id)}" class="{Color.text.fg.b} no-underline card-link">{value.title}</a>
		</h2>
		...
		<div class="icon-bg {Color.text.primary.a}" aria-hidden="true">
			<vector-icon icon="signature"></vector-icon>
		</div>
	</section>
	<footer>
		<img src="{value.cover.src}" alt="{value.cover.alt}" width="800" height="450" />
	</footer>
</article>
