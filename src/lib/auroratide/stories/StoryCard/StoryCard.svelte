<script lang="ts">
    import type { SummarizedStory } from "../StoryType"
	...
	export let value: SummarizedStory
</script>
...
<article aria-labelledby="{value.id}" class="{Theme(value.color)} {Color.bg.bg.b} {Color.border.primary.a} border-after column lift-on-focus">
	<h2 id="{value.id}" class="balance no-margin">
		<a href="{Routes.Stories.href(value.id)}" class="{Color.text.fg.b} no-underline card-link">{value.title}</a>
	</h2>
	<Byline {value} />
